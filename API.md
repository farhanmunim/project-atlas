...
| `GET /api/v1/fleet` | Fleet profile per route — `{ byRoute: { "25": { regs, count, avgAgeYears, propulsion, makes } } }` (DVLA-enriched). Regs are a **daily roster**: build-time live sweep ∪ yesterday's tracked vehicle assignments, so day/school routes are covered despite the overnight build |
