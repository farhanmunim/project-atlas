...
| `GET /api/v1/tenders` | Tender / contract award history per route — `{ byRoute: { "25": [awards…] } }`; each award carries the full published bid spread (`acceptedBid`, `lowestBid`, `highestBid` — TfL's *Accepted / Lowest / Highest Individual Compliant Bid* columns; the accepted bid can sit below the lowest when the winner undercut every individual rival or won as a joint/package award), `numberOfTenderers`, `costPerMile`, `awardDate`, notes, plus derived `jb` (joint-bid partners + total £), `vehicle` (awarded deck/propulsion/basis) and `tranche` |
