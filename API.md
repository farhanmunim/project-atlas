...
| `GET /api/v1/garages` | Bus garages — `{ garages: [{ code, name, operator, company, postcode, lat, lng, pvr, capacity, utilisation, routes, licence? }] }`. `licence` = the PSV operator licence at this operating centre (DVSA VOL register): `{ number, holder, type, status, authorisedVehicles }` — note `authorisedVehicles` is the licence-wide legal ceiling, not per-garage capacity |
| `GET /api/v1/fleet` | Fleet profile per route — `{ byRoute: { "25": { regs, count, avgAgeYears, propulsion, makes } } }` (DVLA-enriched). Regs are a **daily roster**: build-time live sweep ∪ the last 3 tracked days' vehicle assignments, so day/school routes are covered despite the overnight build and a one-day feed outage can't blank a route |
...
| `GET /api/v1/tenders` | Tender / contract award history per route — `{ byRoute: { "25": [awards…] } }`; each award carries the full published bid spread (`acceptedBid`, `lowestBid`, `highestBid` — TfL's *Accepted / Lowest / Highest Individual Compliant Bid* columns; the accepted bid can sit below the lowest when the winner undercut every individual rival or won as a joint/package award), `numberOfTenderers`, `costPerMile`, `awardDate`, notes, plus derived `contractedMilesPA` (= `round(acceptedBid ÷ costPerMile)` — TfL publishes both terms but not the mileage itself), `jb` (joint-bid partners + total £), `vehicle` (awarded deck/propulsion/basis) and `tranche` |
...
| `GET /api/v1/route-diversions` | Active route diversions — `{ count, upcomingFreeze:[ids], routes: { [name]: { id, status, severity, disruptions:[{reason,category,since,until}], since, until, detectedAt, geometryStatus:"published"\|"unpublished", baselineSource:"store"\|"ibus:<ver>", missedStops:{outbound,inbound}, addedStops:{…}, diversionSegments:{outbound:[[[lng,lat],…]],…}, bypassedSegments:{…} } } }`. Built daily by diffing TfL's current Route/Sequence against the frozen canonical baseline (or, when that was polluted by an advance redraw, a dated pre-diversion baseline recovered from TfL's iBus static drops) |
...
`limit` (default 200, max 1000), `offset` (≤100k, pairs with a stable `order`),
`order` (e.g. `day.desc`). Responses:
`{ dataset, count, rows }`.
...
| `GET /api/v1/history/reliability-tracked?route=25` *(filters: `route`, `from`/`to`, `day_type`, `confidence`)* | Our daily **TRACKED** reliability estimate (EWT/OTD v2) — complete observed headways from continuous BODS trip tracking, removing `reliability-daily`'s sampling bias: `ewt_minutes` (= AWT − SWT, high-frequency), `otd_percent` (−2…+5 min, low-frequency) + full breakdown (`on_time`/`early`/`late`/`non_arrival`), `feed_coverage_pct`, `confidence`. Feed outages are unmeasured, never late. **Experimental** — calibrating against TfL's quarterly QSI before promotion |
...
| `GET /api/v1/history/vehicle-sightings?route=25` *(or `?reg=LX58CWU`)* | Vehicle-on-route observations over time (reg ↔ route ↔ timestamp) — once-daily sample |
| `GET /api/v1/history/vehicle-assignments?reg=LX58CWU` *(filters: `reg`, `route`, `from`/`to`)* | Per-vehicle **daily route assignments** from continuous BODS tracking — one row per (reg, route, day) with `first_seen`/`last_seen`, `trips`, `km_observed`; a bus reallocated mid-day shows one row per route it worked |
