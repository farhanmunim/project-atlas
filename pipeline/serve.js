...
const HIST_EP = {
  ...
  "vehicle-assignments": { table: "vehicle_route_assignments_daily", filters: { reg: ["registration", "eq"], route: ["route_id", "eq"], from: ["day", "gte"], to: ["day", "lte"] }, defaultOrder: "day.desc", desc: "Per-vehicle DAILY route assignments from continuous BODS tracking — one row per (reg, route, day) with first/last-seen, trips, observed km; a mid-day reallocation shows one row per route." },
  "reliability-tracked": { table: "route_reliability_tracked_daily", filters: { route: ["route_id", "eq"], from: ["day", "gte"], to: ["day", "lte"], day_type: ["day_type", "eq"], confidence: ["confidence", "eq"] }, defaultOrder: "day.desc", desc: "Atlas's own daily TRACKED reliability estimate (EWT/OTD v2) — EXPERIMENTAL. Complete observed headways from continuous BODS trip tracking; EWT = AWT − SWT, OTD −2…+5 min; feed outages unmeasured." },
};
